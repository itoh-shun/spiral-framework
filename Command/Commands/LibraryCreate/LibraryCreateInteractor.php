...
class LibraryCreateInteractor implements LibraryCreateInteractorInputPortInterface {
    ...
    public function execute(LibraryCreateInteractorInputData $inputdata)
    {
        ...
        if(file_exists('library/'.$inputdata->name))
        {
            throw new Exception('There is already a directory.',1);
        }

        exec("cd library && git submodule add {$inputdata->url} {$inputdata->name} && cd -");

        if(!file_exists('library/makeAutoload.php'))
        {
            $text = getMakeAutoloadFileContent();
            file_put_contents("library/makeAutoload.php", $text);
        }
    }
}
...
function getMakeAutoloadFileContent() {
    return '
    <?php
